...
On login the server issues an opaque bearer token (`username:hex32`), stored as SHA-256 in the
per-user SQLite database. The token has a 365-day lifetime.
...
- Use the stored SHA-256(token) to impersonate you — cannot reverse a SHA-256 hash.
...
The code is open source at https://github.com/Downes/CList.
